...
nbats1 %>% ggplot(aes(x = SitePlot, y = batpass, fill = behavior)) + geom_bar(stat = "identity") + facet_wrap(~manua.lid) + ylim(c(0,200)) 
...
meta <- bats1 %>% dplyr::select(SitePlotYear, PlotType, Detector) %>% unique()
meta1 <- bats1 %>% dplyr::select(manual.id, guild) %>% unique()
