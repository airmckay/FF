...
summary(DF1$guild)
#   LRE   MRE  NoID Noise   SRE 
# 15817 12466   864  2464 77378 

dim(DF1)

# 416936 - Bat passes for both years combined
# 108989 - bat passes manually ID'd
```
...
# 
ggplot(DF1) +
  geom_bar(aes(x = manual.id), stat = "count") +
  dark_mode() + ylab("") + xlab("") + theme(text = element_text(size = 20)) +
  ggtitle("Overview of activity across manual ids")


ggplot(DF1 %>% filter(PlotType == "Open",
                      guild == "SRE")) +
  geom_bar(aes(x = SitePlot), stat = "count") +
  dark_mode() + ylab("") + xlab("") +
  ggtitle("") + facet_wrap(~year)
...
fauto <- ggplot(FF %>% filter(autoid %in% c(
                       "BARBAR", "MYOALC", "MYOBEC", "MYOBRA", 
                     "MYODAS", "MYODAU", "MYOMYO", "MYONAT", 
                     "PLEAUR", "PLEAUS"))) +
  geom_bar(aes(x = SitePlot, fill = Plot), stat = "count") +
  dark_mode() + ylab("") + xlab("") +
  ggtitle("") + facet_wrap(~year) + ylim(c(0,10500))


f22 <- ggplot(DF1 %>% filter(guild == "SRE")) +
  geom_bar(aes(x = SitePlot, fill = PlotType), stat = "count") +
  dark_mode() + ylab("") + xlab("") +
  ggtitle("") + facet_wrap(~year) + ylim(c(0,10500))
...
f21 <- ggplot(FF %>% filter(autoid %in% c(
                       "EPTNIL", "EPTSER", "NYCLEI", "NYCNOC", 
                     "PIPNAT", "PIPPIP", "PIPPYG", "VESMUR",                     year == "2021"))) +
  geom_bar(aes(x = SitePlot), stat = "count") +
  dark_mode() + ylab("") + xlab("") +
  ggtitle("") + facet_wrap(~year)
